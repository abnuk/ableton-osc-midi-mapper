import { OscParameterType, OscParameterDef } from '../../domain/entities/OscCommand';

/**
 * OSC Command Category
 */
export enum OscCommandCategory {
  SONG = 'song',
  TRACK = 'track',
  CLIP = 'clip',
  CLIP_SLOT = 'clip_slot',
  SCENE = 'scene',
  DEVICE = 'device',
  VIEW = 'view',
  APPLICATION = 'application'
}

/**
 * OSC Command Definition
 */
export interface OscCommandDef {
  address: string;
  category: OscCommandCategory;
  name: string;
  description: string;
  parameters: OscParameterDef[];
  tags?: string[];
}

/**
 * Comprehensive OSC Command Catalog
 * Extracted from AbletonOSC documentation
 */
export class OscCommandCatalog {
  private static commands: OscCommandDef[] = [
    // ==================== SONG API ====================
    {
      address: '/live/song/start_playing',
      category: OscCommandCategory.SONG,
      name: 'Start Playing',
      description: 'Start session playback',
      parameters: []
    },
    {
      address: '/live/song/stop_playing',
      category: OscCommandCategory.SONG,
      name: 'Stop Playing',
      description: 'Stop session playback',
      parameters: []
    },
    {
      address: '/live/song/continue_playing',
      category: OscCommandCategory.SONG,
      name: 'Continue Playing',
      description: 'Resume session playback',
      parameters: []
    },
    {
      address: '/live/song/stop_all_clips',
      category: OscCommandCategory.SONG,
      name: 'Stop All Clips',
      description: 'Stop all clips from playing',
      parameters: []
    },
    {
      address: '/live/song/set/tempo',
      category: OscCommandCategory.SONG,
      name: 'Set Tempo',
      description: 'Set the song tempo',
      parameters: [
        { name: 'tempo', type: OscParameterType.FLOAT, optional: false, description: 'Tempo in BPM', min: 20, max: 999 }
      ]
    },
    {
      address: '/live/song/set/metronome',
      category: OscCommandCategory.SONG,
      name: 'Set Metronome',
      description: 'Enable/disable metronome',
      parameters: [
        { name: 'enabled', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/song/create_audio_track',
      category: OscCommandCategory.SONG,
      name: 'Create Audio Track',
      description: 'Create a new audio track',
      parameters: [
        { name: 'index', type: OscParameterType.INTEGER, optional: false, description: 'Track index (-1 for end)', min: -1 }
      ]
    },
    {
      address: '/live/song/create_midi_track',
      category: OscCommandCategory.SONG,
      name: 'Create MIDI Track',
      description: 'Create a new MIDI track',
      parameters: [
        { name: 'index', type: OscParameterType.INTEGER, optional: false, description: 'Track index (-1 for end)', min: -1 }
      ]
    },
    {
      address: '/live/song/delete_track',
      category: OscCommandCategory.SONG,
      name: 'Delete Track',
      description: 'Delete a track',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 }
      ]
    },
    {
      address: '/live/song/duplicate_track',
      category: OscCommandCategory.SONG,
      name: 'Duplicate Track',
      description: 'Duplicate a track',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 }
      ]
    },
    {
      address: '/live/song/create_scene',
      category: OscCommandCategory.SONG,
      name: 'Create Scene',
      description: 'Create a new scene',
      parameters: [
        { name: 'index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index (-1 for end)', min: -1 }
      ]
    },
    {
      address: '/live/song/delete_scene',
      category: OscCommandCategory.SONG,
      name: 'Delete Scene',
      description: 'Delete a scene',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 }
      ]
    },
    {
      address: '/live/song/duplicate_scene',
      category: OscCommandCategory.SONG,
      name: 'Duplicate Scene',
      description: 'Duplicate a scene',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 }
      ]
    },
    {
      address: '/live/song/undo',
      category: OscCommandCategory.SONG,
      name: 'Undo',
      description: 'Undo the last operation',
      parameters: []
    },
    {
      address: '/live/song/redo',
      category: OscCommandCategory.SONG,
      name: 'Redo',
      description: 'Redo the last undone operation',
      parameters: []
    },
    {
      address: '/live/song/tap_tempo',
      category: OscCommandCategory.SONG,
      name: 'Tap Tempo',
      description: 'Mimics a tap of the "Tap Tempo" button',
      parameters: []
    },
    {
      address: '/live/song/trigger_session_record',
      category: OscCommandCategory.SONG,
      name: 'Trigger Session Record',
      description: 'Triggers record in session mode',
      parameters: []
    },
    {
      address: '/live/song/capture_midi',
      category: OscCommandCategory.SONG,
      name: 'Capture MIDI',
      description: 'Capture MIDI',
      parameters: []
    },
    {
      address: '/live/song/create_return_track',
      category: OscCommandCategory.SONG,
      name: 'Create Return Track',
      description: 'Create a new return track',
      parameters: []
    },
    {
      address: '/live/song/delete_return_track',
      category: OscCommandCategory.SONG,
      name: 'Delete Return Track',
      description: 'Delete a return track',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Return track index', min: 0 }
      ]
    },
    {
      address: '/live/song/jump_by',
      category: OscCommandCategory.SONG,
      name: 'Jump By',
      description: 'Jump song position by specified time in beats',
      parameters: [
        { name: 'beats', type: OscParameterType.FLOAT, optional: false, description: 'Time to jump in beats' }
      ]
    },
    {
      address: '/live/song/jump_to_next_cue',
      category: OscCommandCategory.SONG,
      name: 'Jump to Next Cue',
      description: 'Jump to the next cue marker',
      parameters: []
    },
    {
      address: '/live/song/jump_to_prev_cue',
      category: OscCommandCategory.SONG,
      name: 'Jump to Previous Cue',
      description: 'Jump to the previous cue marker',
      parameters: []
    },
    {
      address: '/live/song/set/arrangement_overdub',
      category: OscCommandCategory.SONG,
      name: 'Set Arrangement Overdub',
      description: 'Set arrangement overdub on/off',
      parameters: [
        { name: 'enabled', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/song/set/back_to_arranger',
      category: OscCommandCategory.SONG,
      name: 'Set Back to Arranger',
      description: 'Set whether "back to arranger" is lit',
      parameters: [
        { name: 'enabled', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/song/set/clip_trigger_quantization',
      category: OscCommandCategory.SONG,
      name: 'Set Clip Trigger Quantization',
      description: 'Set clip trigger quantization level',
      parameters: [
        { name: 'quantization', type: OscParameterType.INTEGER, optional: false, description: 'Quantization level', min: 0 }
      ]
    },
    {
      address: '/live/song/set/current_song_time',
      category: OscCommandCategory.SONG,
      name: 'Set Current Song Time',
      description: 'Set current song time in beats',
      parameters: [
        { name: 'time', type: OscParameterType.FLOAT, optional: false, description: 'Time in beats', min: 0 }
      ]
    },
    {
      address: '/live/song/set/groove_amount',
      category: OscCommandCategory.SONG,
      name: 'Set Groove Amount',
      description: 'Set groove amount',
      parameters: [
        { name: 'amount', type: OscParameterType.FLOAT, optional: false, description: 'Groove amount 0.0-1.0', min: 0, max: 1 }
      ]
    },
    {
      address: '/live/song/set/loop',
      category: OscCommandCategory.SONG,
      name: 'Set Loop',
      description: 'Set song loop on/off',
      parameters: [
        { name: 'enabled', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/song/set/loop_start',
      category: OscCommandCategory.SONG,
      name: 'Set Loop Start',
      description: 'Set loop start point',
      parameters: [
        { name: 'start', type: OscParameterType.FLOAT, optional: false, description: 'Loop start in beats', min: 0 }
      ]
    },
    {
      address: '/live/song/set/loop_length',
      category: OscCommandCategory.SONG,
      name: 'Set Loop Length',
      description: 'Set loop length',
      parameters: [
        { name: 'length', type: OscParameterType.FLOAT, optional: false, description: 'Loop length in beats', min: 0 }
      ]
    },
    {
      address: '/live/song/set/midi_recording_quantization',
      category: OscCommandCategory.SONG,
      name: 'Set MIDI Recording Quantization',
      description: 'Set MIDI recording quantization',
      parameters: [
        { name: 'quantization', type: OscParameterType.INTEGER, optional: false, description: 'Quantization level', min: 0 }
      ]
    },
    {
      address: '/live/song/set/punch_in',
      category: OscCommandCategory.SONG,
      name: 'Set Punch In',
      description: 'Set punch in on/off',
      parameters: [
        { name: 'enabled', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/song/set/punch_out',
      category: OscCommandCategory.SONG,
      name: 'Set Punch Out',
      description: 'Set punch out on/off',
      parameters: [
        { name: 'enabled', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/song/set/record_mode',
      category: OscCommandCategory.SONG,
      name: 'Set Record Mode',
      description: 'Set record mode',
      parameters: [
        { name: 'mode', type: OscParameterType.INTEGER, optional: false, description: 'Record mode', min: 0 }
      ]
    },
    {
      address: '/live/song/set/session_record',
      category: OscCommandCategory.SONG,
      name: 'Set Session Record',
      description: 'Set session record on/off',
      parameters: [
        { name: 'enabled', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/song/set/signature_numerator',
      category: OscCommandCategory.SONG,
      name: 'Set Time Signature Numerator',
      description: 'Set time signature numerator',
      parameters: [
        { name: 'numerator', type: OscParameterType.INTEGER, optional: false, description: 'Numerator', min: 1 }
      ]
    },
    {
      address: '/live/song/set/signature_denominator',
      category: OscCommandCategory.SONG,
      name: 'Set Time Signature Denominator',
      description: 'Set time signature denominator',
      parameters: [
        { name: 'denominator', type: OscParameterType.INTEGER, optional: false, description: 'Denominator', min: 1 }
      ]
    },

    // ==================== TRACK API (ADDITIONAL SETTERS) ====================
    {
      address: '/live/track/set/volume',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Volume',
      description: 'Set track volume',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'volume', type: OscParameterType.FLOAT, optional: false, description: 'Volume (0.0-1.0)', min: 0, max: 1 }
      ],
      tags: ['mixer']
    },
    {
      address: '/live/track/set/panning',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Panning',
      description: 'Set track panning',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'panning', type: OscParameterType.FLOAT, optional: false, description: 'Pan (-1.0 to 1.0)', min: -1, max: 1 }
      ],
      tags: ['mixer']
    },
    {
      address: '/live/track/set/send',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Send',
      description: 'Set track send level',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'send_index', type: OscParameterType.INTEGER, optional: false, description: 'Send index', min: 0 },
        { name: 'value', type: OscParameterType.FLOAT, optional: false, description: 'Send level (0.0-1.0)', min: 0, max: 1 }
      ],
      tags: ['mixer']
    },
    {
      address: '/live/track/set/mute',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Mute',
      description: 'Mute/unmute a track',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'mute', type: OscParameterType.BOOLEAN, optional: false, description: '1=mute, 0=unmute' }
      ]
    },
    {
      address: '/live/track/set/solo',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Solo',
      description: 'Solo/unsolo a track',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'solo', type: OscParameterType.BOOLEAN, optional: false, description: '1=solo, 0=unsolo' }
      ]
    },
    {
      address: '/live/track/set/arm',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Arm',
      description: 'Arm/unarm a track for recording',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'arm', type: OscParameterType.BOOLEAN, optional: false, description: '1=arm, 0=unarm' }
      ]
    },
    {
      address: '/live/track/set/name',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Name',
      description: 'Set track name',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'name', type: OscParameterType.STRING, optional: false, description: 'Track name' }
      ]
    },
    {
      address: '/live/track/stop_all_clips',
      category: OscCommandCategory.TRACK,
      name: 'Stop All Clips on Track',
      description: 'Stop all clips on a track',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 }
      ]
    },
    {
      address: '/live/track/set/color',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Color',
      description: 'Set track color',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'color', type: OscParameterType.INTEGER, optional: false, description: 'Color value', min: 0 }
      ]
    },
    {
      address: '/live/track/set/color_index',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Color Index',
      description: 'Set track color index',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'color_index', type: OscParameterType.INTEGER, optional: false, description: 'Color index', min: 0 }
      ]
    },
    {
      address: '/live/track/set/current_monitoring_state',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Monitoring State',
      description: 'Set track monitoring on/off',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'state', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/track/set/fold_state',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Fold State',
      description: 'Set group track folded state',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'fold_state', type: OscParameterType.BOOLEAN, optional: false, description: '1=folded, 0=unfolded' }
      ]
    },
    {
      address: '/live/track/set/input_routing_channel',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Input Routing Channel',
      description: 'Set input routing channel',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'channel', type: OscParameterType.STRING, optional: false, description: 'Channel name' }
      ]
    },
    {
      address: '/live/track/set/input_routing_type',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Input Routing Type',
      description: 'Set input routing type',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'type', type: OscParameterType.STRING, optional: false, description: 'Routing type' }
      ]
    },
    {
      address: '/live/track/set/output_routing_channel',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Output Routing Channel',
      description: 'Set output routing channel',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'channel', type: OscParameterType.STRING, optional: false, description: 'Channel name' }
      ]
    },
    {
      address: '/live/track/set/output_routing_type',
      category: OscCommandCategory.TRACK,
      name: 'Set Track Output Routing Type',
      description: 'Set output routing type',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'type', type: OscParameterType.STRING, optional: false, description: 'Routing type' }
      ]
    },

    // ==================== CLIP SLOT API ====================
    {
      address: '/live/clip_slot/fire',
      category: OscCommandCategory.CLIP_SLOT,
      name: 'Fire Clip Slot',
      description: 'Fire/play clip in slot',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip slot index', min: 0 }
      ]
    },
    {
      address: '/live/clip_slot/stop',
      category: OscCommandCategory.CLIP_SLOT,
      name: 'Stop Clip Slot',
      description: 'Stop clip in slot',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip slot index', min: 0 }
      ]
    },
    {
      address: '/live/clip_slot/delete_clip',
      category: OscCommandCategory.CLIP_SLOT,
      name: 'Delete Clip',
      description: 'Delete clip in slot',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip slot index', min: 0 }
      ],
      tags: ['important']
    },
    {
      address: '/live/clip_slot/create_clip',
      category: OscCommandCategory.CLIP_SLOT,
      name: 'Create Clip',
      description: 'Create a new clip in slot',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip slot index', min: 0 },
        { name: 'length', type: OscParameterType.FLOAT, optional: false, description: 'Clip length in beats', min: 0.25 }
      ]
    },
    {
      address: '/live/clip_slot/duplicate_clip_to',
      category: OscCommandCategory.CLIP_SLOT,
      name: 'Duplicate Clip To',
      description: 'Duplicate clip to target slot',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Source track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Source clip index', min: 0 },
        { name: 'target_track_index', type: OscParameterType.INTEGER, optional: false, description: 'Target track index', min: 0 },
        { name: 'target_clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Target clip index', min: 0 }
      ]
    },
    {
      address: '/live/clip_slot/set/has_stop_button',
      category: OscCommandCategory.CLIP_SLOT,
      name: 'Set Has Stop Button',
      description: 'Add or remove stop button',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip slot index', min: 0 },
        { name: 'has_stop_button', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },

    // ==================== CLIP API ====================
    {
      address: '/live/clip/fire',
      category: OscCommandCategory.CLIP,
      name: 'Fire Clip',
      description: 'Start clip playback',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 }
      ]
    },
    {
      address: '/live/clip/stop',
      category: OscCommandCategory.CLIP,
      name: 'Stop Clip',
      description: 'Stop clip playback',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 }
      ]
    },
    {
      address: '/live/clip/set/name',
      category: OscCommandCategory.CLIP,
      name: 'Set Clip Name',
      description: 'Set clip name',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'name', type: OscParameterType.STRING, optional: false, description: 'Clip name' }
      ]
    },
    {
      address: '/live/clip/set/color',
      category: OscCommandCategory.CLIP,
      name: 'Set Clip Color',
      description: 'Set clip color',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'color', type: OscParameterType.INTEGER, optional: false, description: 'Color value', min: 0 }
      ]
    },
    {
      address: '/live/clip/set/gain',
      category: OscCommandCategory.CLIP,
      name: 'Set Clip Gain',
      description: 'Set clip gain',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'gain', type: OscParameterType.FLOAT, optional: false, description: 'Gain value', min: 0, max: 2 }
      ]
    },
    {
      address: '/live/clip/duplicate_loop',
      category: OscCommandCategory.CLIP,
      name: 'Duplicate Clip Loop',
      description: 'Duplicates clip loop',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 }
      ]
    },
    {
      address: '/live/clip/set/pitch_coarse',
      category: OscCommandCategory.CLIP,
      name: 'Set Clip Pitch Coarse',
      description: 'Set clip coarse re-pitch in semitones',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'semitones', type: OscParameterType.INTEGER, optional: false, description: 'Semitones', min: -48, max: 48 }
      ]
    },
    {
      address: '/live/clip/set/pitch_fine',
      category: OscCommandCategory.CLIP,
      name: 'Set Clip Pitch Fine',
      description: 'Set clip fine re-pitch in cents',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'cents', type: OscParameterType.INTEGER, optional: false, description: 'Cents', min: -50, max: 50 }
      ]
    },
    {
      address: '/live/clip/set/loop_start',
      category: OscCommandCategory.CLIP,
      name: 'Set Clip Loop Start',
      description: 'Set clip loop start',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'loop_start', type: OscParameterType.FLOAT, optional: false, description: 'Loop start in beats', min: 0 }
      ]
    },
    {
      address: '/live/clip/set/loop_end',
      category: OscCommandCategory.CLIP,
      name: 'Set Clip Loop End',
      description: 'Set clip loop end',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'loop_end', type: OscParameterType.FLOAT, optional: false, description: 'Loop end in beats', min: 0 }
      ]
    },
    {
      address: '/live/clip/set/warping',
      category: OscCommandCategory.CLIP,
      name: 'Set Clip Warping',
      description: 'Set clip warp mode on/off',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'warping', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/clip/set/start_marker',
      category: OscCommandCategory.CLIP,
      name: 'Set Clip Start Marker',
      description: 'Set clip start marker in beats',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'start_marker', type: OscParameterType.FLOAT, optional: false, description: 'Start marker in beats', min: 0 }
      ]
    },
    {
      address: '/live/clip/set/end_marker',
      category: OscCommandCategory.CLIP,
      name: 'Set Clip End Marker',
      description: 'Set clip end marker in beats',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'end_marker', type: OscParameterType.FLOAT, optional: false, description: 'End marker in beats', min: 0 }
      ]
    },
    {
      address: '/live/clip/add/notes',
      category: OscCommandCategory.CLIP,
      name: 'Add MIDI Notes',
      description: 'Add MIDI notes to clip',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'pitch', type: OscParameterType.INTEGER, optional: false, description: 'MIDI note (0-127)', min: 0, max: 127 },
        { name: 'start_time', type: OscParameterType.FLOAT, optional: false, description: 'Start time in beats', min: 0 },
        { name: 'duration', type: OscParameterType.FLOAT, optional: false, description: 'Duration in beats', min: 0 },
        { name: 'velocity', type: OscParameterType.INTEGER, optional: false, description: 'Velocity (0-127)', min: 0, max: 127 },
        { name: 'mute', type: OscParameterType.BOOLEAN, optional: false, description: '1=muted, 0=unmuted' }
      ]
    },
    {
      address: '/live/clip/remove/notes',
      category: OscCommandCategory.CLIP,
      name: 'Remove MIDI Notes',
      description: 'Remove MIDI notes from clip',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'clip_index', type: OscParameterType.INTEGER, optional: false, description: 'Clip index', min: 0 },
        { name: 'start_pitch', type: OscParameterType.INTEGER, optional: true, description: 'Start pitch', min: 0, max: 127 },
        { name: 'pitch_span', type: OscParameterType.INTEGER, optional: true, description: 'Pitch span', min: 0 },
        { name: 'start_time', type: OscParameterType.FLOAT, optional: true, description: 'Start time in beats', min: 0 },
        { name: 'time_span', type: OscParameterType.FLOAT, optional: true, description: 'Time span in beats', min: 0 }
      ]
    },

    // ==================== SCENE API ====================
    {
      address: '/live/scene/fire',
      category: OscCommandCategory.SCENE,
      name: 'Fire Scene',
      description: 'Trigger a scene',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 }
      ]
    },
    {
      address: '/live/scene/fire_as_selected',
      category: OscCommandCategory.SCENE,
      name: 'Fire Scene as Selected',
      description: 'Fire scene and select next',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 }
      ]
    },
    {
      address: '/live/scene/set/name',
      category: OscCommandCategory.SCENE,
      name: 'Set Scene Name',
      description: 'Set scene name',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 },
        { name: 'name', type: OscParameterType.STRING, optional: false, description: 'Scene name' }
      ]
    },
    {
      address: '/live/scene/set/color',
      category: OscCommandCategory.SCENE,
      name: 'Set Scene Color',
      description: 'Set scene color',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 },
        { name: 'color', type: OscParameterType.INTEGER, optional: false, description: 'Color value', min: 0 }
      ]
    },
    {
      address: '/live/scene/set/color_index',
      category: OscCommandCategory.SCENE,
      name: 'Set Scene Color Index',
      description: 'Set scene color index',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 },
        { name: 'color_index', type: OscParameterType.INTEGER, optional: false, description: 'Color index', min: 0 }
      ]
    },
    {
      address: '/live/scene/set/tempo',
      category: OscCommandCategory.SCENE,
      name: 'Set Scene Tempo',
      description: 'Set scene tempo',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 },
        { name: 'tempo', type: OscParameterType.FLOAT, optional: false, description: 'Tempo in BPM', min: 20, max: 999 }
      ]
    },
    {
      address: '/live/scene/set/tempo_enabled',
      category: OscCommandCategory.SCENE,
      name: 'Set Scene Tempo Enabled',
      description: 'Set whether scene tempo is enabled',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 },
        { name: 'enabled', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/scene/set/time_signature_numerator',
      category: OscCommandCategory.SCENE,
      name: 'Set Scene Time Signature Numerator',
      description: 'Set scene time signature numerator',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 },
        { name: 'numerator', type: OscParameterType.INTEGER, optional: false, description: 'Numerator', min: 1 }
      ]
    },
    {
      address: '/live/scene/set/time_signature_denominator',
      category: OscCommandCategory.SCENE,
      name: 'Set Scene Time Signature Denominator',
      description: 'Set scene time signature denominator',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 },
        { name: 'denominator', type: OscParameterType.INTEGER, optional: false, description: 'Denominator', min: 1 }
      ]
    },
    {
      address: '/live/scene/set/time_signature_enabled',
      category: OscCommandCategory.SCENE,
      name: 'Set Scene Time Signature Enabled',
      description: 'Set whether scene time signature is enabled',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 },
        { name: 'enabled', type: OscParameterType.BOOLEAN, optional: false, description: '1=on, 0=off' }
      ]
    },
    {
      address: '/live/scene/fire_selected',
      category: OscCommandCategory.SCENE,
      name: 'Fire Selected Scene',
      description: 'Trigger selected scene and select next',
      parameters: []
    },

    // ==================== VIEW API ====================
    {
      address: '/live/view/set/selected_track',
      category: OscCommandCategory.VIEW,
      name: 'Select Track',
      description: 'Set selected track',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 }
      ]
    },
    {
      address: '/live/view/set/selected_scene',
      category: OscCommandCategory.VIEW,
      name: 'Select Scene',
      description: 'Set selected scene',
      parameters: [
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 }
      ]
    },
    {
      address: '/live/view/set/selected_clip',
      category: OscCommandCategory.VIEW,
      name: 'Select Clip',
      description: 'Set selected clip',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'scene_index', type: OscParameterType.INTEGER, optional: false, description: 'Scene index', min: 0 }
      ]
    },
    {
      address: '/live/view/set/selected_device',
      category: OscCommandCategory.VIEW,
      name: 'Select Device',
      description: 'Set selected device',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'device_index', type: OscParameterType.INTEGER, optional: false, description: 'Device index', min: 0 }
      ]
    },

    // ==================== DEVICE API ====================
    {
      address: '/live/device/set/parameter/value',
      category: OscCommandCategory.DEVICE,
      name: 'Set Device Parameter',
      description: 'Set device parameter value',
      parameters: [
        { name: 'track_index', type: OscParameterType.INTEGER, optional: false, description: 'Track index', min: 0 },
        { name: 'device_index', type: OscParameterType.INTEGER, optional: false, description: 'Device index', min: 0 },
        { name: 'parameter_index', type: OscParameterType.INTEGER, optional: false, description: 'Parameter index', min: 0 },
        { name: 'value', type: OscParameterType.FLOAT, optional: false, description: 'Parameter value' }
      ]
    }
  ];

  /**
   * Get all commands
   */
  static getAllCommands(): ReadonlyArray<OscCommandDef> {
    return this.commands;
  }

  /**
   * Get commands by category
   */
  static getCommandsByCategory(category: OscCommandCategory): OscCommandDef[] {
    return this.commands.filter(cmd => cmd.category === category);
  }

  /**
   * Get command by address
   */
  static getCommandByAddress(address: string): OscCommandDef | undefined {
    return this.commands.find(cmd => cmd.address === address);
  }

  /**
   * Search commands by name or description
   */
  static searchCommands(query: string): OscCommandDef[] {
    const lowerQuery = query.toLowerCase();
    return this.commands.filter(
      cmd =>
        cmd.name.toLowerCase().includes(lowerQuery) ||
        cmd.description.toLowerCase().includes(lowerQuery) ||
        cmd.address.toLowerCase().includes(lowerQuery)
    );
  }

  /**
   * Get commands by tags
   */
  static getCommandsByTag(tag: string): OscCommandDef[] {
    return this.commands.filter(cmd => cmd.tags?.includes(tag));
  }

  /**
   * Get all categories
   */
  static getAllCategories(): OscCommandCategory[] {
    return Object.values(OscCommandCategory);
  }
}

